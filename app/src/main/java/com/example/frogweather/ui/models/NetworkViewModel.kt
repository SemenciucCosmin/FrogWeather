...
class NetworkViewModel : ViewModel() {
        ...
        viewModelScope.launch {
            when (val dailyResult = networkRepository.getDailyForecast(latitude, longitude)) {
                is CallResult.Success ->
                    when (val hourlyResult = networkRepository.getHourlyForecast(latitude, longitude)) {
                        is CallResult.Success ->
                            _uiState.value = _uiState.value?.copy(
                                tenDaysItems = getTenDaysListItems(settings, dailyResult.data.list, hourlyResult.data.list),
                                oneDayItems = getOneDayListItems(dayType, settings, dailyResult.data.list, hourlyResult.data.list),
                                errorMessageTenDays = null
                            )
                        is CallResult.Error -> _uiState.value = _uiState.value?.copy(errorMessageTenDays = CONNECTION_ERROR_MESSAGE)
                    }
                is CallResult.Error -> _uiState.value = _uiState.value?.copy(errorMessageTenDays = CONNECTION_ERROR_MESSAGE)
            }
        }
    ...
    private fun getOneDayListItems(
        ...
        if (dayType != null) {
            ...
            if (forecasts.isNotEmpty()) {
                val currentForecast = getCurrentForecastDataClass(forecasts, settings, DayType.getByDayType(dayType), dailyForecasts)
                ...
                return listOf(currentForecast, hourlyForecast, chance, details, hourlyPrecipitation, hourlyWind)
            }
        }
        return listOf()
    }
    ...
    private fun getCurrentForecastDataClass(
        ...
        val time = if (settings.hourFormatUnit) {
            SimpleDateFormat(AM_PM_TIME_PATTERN, Locale.ENGLISH).format(Date(System.currentTimeMillis()))
        } else {
            SimpleDateFormat(ALL_HOURS_TIME_PATTERN, Locale.ENGLISH).format(Date(System.currentTimeMillis()))
        }
        val minTemperature =
        ...
        val currentTemperature =
            TemperatureType.convertFromKelvin(forecasts.firstOrNull()?.measurements?.temp?.roundToInt() ?: 0, settings.temperatureUnit)
        val feelsLike =
            TemperatureType.convertFromKelvin(forecasts.firstOrNull()?.measurements?.feelsLike?.roundToInt() ?: 0, settings.temperatureUnit)
    ...
    private fun getHourlyForecastsDataClass(forecasts: List<ForecastHourly>, settings: Settings): OneDayListItem.HourlyForecastDataClass {
        val timeFormatter = SimpleDateFormat(ALL_HOURS_TIME_PATTERN, Locale.ENGLISH)
        return OneDayListItem.HourlyForecastDataClass(forecasts.map { forecast ->
            ForecastDetail(
                TemperatureType.convertFromKelvin(forecast.measurements.temp.roundToInt(), settings.temperatureUnit),
    ...
    private fun getChanceDataClass(dayType: DayType, dailyForecasts: List<ForecastDaily>?) =
        OneDayListItem.ChanceDataClass(((dailyForecasts?.get(dayType.dayPosition)?.pop)?.times(100))?.roundToInt() ?: 0)

    private fun getDetailsDataClass(forecast: ForecastHourly, settings: Settings): OneDayListItem.DetailsDataClass {
    ...
    private fun getHourlyPrecipitationsDataClass(forecasts: List<ForecastHourly>, settings: Settings): OneDayListItem.PrecipitationDataClass {
        val timeFormatter = SimpleDateFormat(ALL_HOURS_TIME_PATTERN, Locale.ENGLISH)
        return OneDayListItem.PrecipitationDataClass(forecasts.map { forecast ->
            PrecipitationDetail(
                (forecast.pop * 100).roundToInt(),
                LengthType.convertFromMillimeters(forecast.rain.value, settings.lengthUnit),
                timeFormatter.format(Date(forecast.dt * 1000)),
                getPrecipitationIconResourceId(LengthType.convertFromMillimeters(forecast.rain.value, settings.lengthUnit))
            )
        }, LengthType.getByLengthType(settings.lengthUnit).shortResourceId)
    }

    private fun getHourlyWindDataClass(forecasts: List<ForecastHourly>, settings: Settings): OneDayListItem.WindDataClass {
        val timeFormatter = SimpleDateFormat(ALL_HOURS_TIME_PATTERN, Locale.ENGLISH)
        return OneDayListItem.WindDataClass(forecasts.map { forecast ->
            WindDetail(
    ...
    private fun getForecastsByDayType(dayType: DayType, forecasts: List<ForecastHourly>?): List<ForecastHourly> {
        ...
        if (dailyForecasts != null && hourlyForecasts != null) {
            ...
            return dailyForecasts.map { forecast ->
                TenDaysListItem(
                    ...
                    (forecast.pop * 100).roundToInt(),
                    TemperatureType.convertFromKelvin(forecast.temperature.max.roundToInt(), settings.temperatureUnit),
                    TemperatureType.convertFromKelvin(forecast.temperature.min.roundToInt(), settings.temperatureUnit),
                    ...
                    PressureType.getByPressureType(settings.pressureUnit).valueResourceId,
                    forecast.humidity,
                    SimpleDateFormat(timePattern, Locale.ENGLISH).format(Date(forecast.sunrise * 1000)),
                    SimpleDateFormat(timePattern, Locale.ENGLISH).format(Date(forecast.sunset * 1000)),
                    hourlyForecasts.filter {
                        SimpleDateFormat(DATE_PATTERN, Locale.ENGLISH).format(Date(it.dt * 1000)) == SimpleDateFormat(
                            DATE_PATTERN,
                            Locale.ENGLISH
                        ).format(Date(forecast.dt * 1000))
                    }.map { hourlyForecast ->
                        ForecastDetail(
                            TemperatureType.convertFromKelvin(hourlyForecast.measurements.temp.roundToInt(), settings.temperatureUnit),
                            0,
                            SimpleDateFormat(ALL_HOURS_TIME_PATTERN, Locale.ENGLISH).format(Date(hourlyForecast.dt * 1000)),
                        )
                    }
                )
            }
    ...
    fun changeTenDaysListItemState(tenDaysListItem: TenDaysListItem) {
        ...
        _uiState.value = _uiState.value?.copy(tenDaysItems = newList?.map {
